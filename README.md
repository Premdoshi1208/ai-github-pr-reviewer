...
5. Save it for Phase 7

### 4.4 LangFuse Account

LangFuse records every single Groq API call your system makes. You can see what prompt was sent, what came back, how many tokens were used, and what it cost. It is free.
...
This prints a 12-digit number like `604175033417`. Save this number — it is your `AWS_ACCOUNT_ID`. You will need it in later steps.
...
13. Copy the Role ARN at the top — it looks like:
    `arn:aws:iam::604175033417:role/github-actions-ai-reviewer`
...
ecr_gateway_url        = "604175033417.dkr.ecr.us-east-1.amazonaws.com/gateway"
ecr_webhook_url        = "604175033417.dkr.ecr.us-east-1.amazonaws.com/webhook"
ecr_orchestrator_url   = "604175033417.dkr.ecr.us-east-1.amazonaws.com/orchestrator"
ecr_reviewer_url       = "604175033417.dkr.ecr.us-east-1.amazonaws.com/reviewer"
ecr_learner_url        = "604175033417.dkr.ecr.us-east-1.amazonaws.com/learner"
```
...
| `AWS_ROLE_ARN` | the full ARN from Step 6.5 — looks like `arn:aws:iam::604175033417:role/github-actions-ai-reviewer` |
...
You will see:
```
Added new context arn:aws:eks:us-east-1:604175033417:cluster/ai-code-reviewer to C:\Users\YourName\.kube\config
```
...
- **Generations** — every individual Groq API call with full prompt and response
...
Common causes:
- Groq API key is invalid — check console.groq.com to see if calls are being made
- Groq account has no available quota — check your Groq dashboard and limits
...
| Resource | Cost |
|---|---|
| Groq `llama-3.1-8b-instant` | Free tier available; usage depends on Groq plan limits |
